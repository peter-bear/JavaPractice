...
public class BinarySearchTree {
	...
	public int countOneChildParents() {
		...
		return root.countOneChildParent();
	}
	...
	public Integer branchPoint(Integer x, Integer y) {
		// TODO: Implement this method!
		BinaryNode tmp = new BinaryNode(null);
	    root.getBrachPoint(Math.min(x, y), Math.max(x, y), tmp);
	    return tmp.data;
	}
	...
	public class BinaryNode {
		...
		public int countOneChildParent() {
			if (this == NULL_NODE ) 
				return 0;
			
			int cnt = 0;
			if((this.left == NULL_NODE && this.right != NULL_NODE) || (this.left != NULL_NODE && this.right == NULL_NODE))
				cnt += 1;
			
			return cnt + this.left.countOneChildParent() + this.right.countOneChildParent();
		}
		
		public BinaryNode removeLeavers() {
			if(this == NULL_NODE || (this.left == NULL_NODE && this.right == NULL_NODE))
				return NULL_NODE;
			
			
			this.left = this.left.removeLeavers();
			this.right = this.right.removeLeavers();
			
			return this;
		}
		
		public void getBrachPoint(Integer x, Integer y, BinaryNode ancesstor) {
			if(this == NULL_NODE)
				return ;
			
			
			if(x <= this.data && y >= this.data) {
				ancesstor.data = this.data;
				return;
				
				
			}else if(x > this.data) {
				this.right.getBrachPoint(x, y, ancesstor);
			}else {
				this.left.getBrachPoint(x, y, ancesstor);
			}
